...
#		self.loadmenu()
 ...
 def cycleControl(self, task):
	...
	entries.sort(lambda x,y: cmp(y.getSurfacePoint(render).getZ(),
                                     x.getSurfacePoint(render).getZ()))
	if (len(entries)>0) and ((entries[0].getIntoNode().getName() == "CityTerrain") or (entries[0].getIntoNode().getName() == "wide_ramp") or (entries[0].getIntoNode().getName() == "rail_ramp") or (entries[0].getIntoNode().getName() == "bridge") ):
		base.camera.setZ(entries[0].getSurfacePoint(self.track).getZ()+1.0)
		#print("idhar",entries[0].getSurfacePoint(self.track))
	if (base.camera.getZ() < self.cycle.getZ() + 2.0):
		base.camera.setZ(self.cycle.getZ() + 2.0)
		#print("udhar" )
	if (base.camera.getZ() > self.cycle.getZ() + 2.0):
		base.camera.setZ(self.cycle.getZ() + 2.0)
		#print("idhar")
	self.floater.setPos(self.cycle.getPos())
	self.floater.setZ(self.cycle.getZ() + 2.0)
 ...
 def bikecollideEventOut(self, entry):
  ...
  #self.speed = 150
  self.blood = Light()
  self.blood.loadchar(self.cycle,'blood')
  self.diesound()
  self.floating = False
  #dt = globalClock.getDt()
  self.jump1()
 ...
 def jump1(self):
	...
	if not self.floating:
		...
		toData = self.cycle.getPos()+Vec3(4,4,4), duration = 0.6,
		blendType = 'easeOut'
		)
		self.seq=Sequence(lf, Wait(299.7))
		self.seq.setPlayRate(4)
 ...
 def setlig(self, task):
	for i in range(len(self.lm)):
		if(((abs(abs(self.cycle.getX())) -(abs(self.lm[i].getX()))) >= 250) and ((abs((abs(self.cycle.getY()))-(abs(self.lm[i].getY()))))>=250) ):
			#print(self.lm[i],self.lm[i].getPos())
			self.lm[i].detachNode()
			#self.lm[i].setColor(0.4,0.4,0.4,0.4)
		#else:
	...
	for i in range(len(self.treeModel1)):
		...
		#else:
			#self.lm[i].reparentTo(self.track)
		if(((abs(abs(self.cycle.getX())) -(abs(self.treeModel1[i].getX()))) > 100) and ((abs((abs(self.cycle.getY()))-(abs(self.treeModel1[i].getY()))))>100) ):
			#print(self.heartModel1[i],self.heartModel1[i].getPos())
 ...
 def createmap(self ):
	...
	self.lightpivot31 = render.attachNewNode("lightpivot31")
	self.lightpivot31.setPos(0,0,1)
	self.lightpivot31.hprInterval(1,Vec3(360,0,0)).loop()
	#self.lightpivot31.attachNewNode(self.h1)
	self.h1 = loader.loadModel("Models/health/health")
	self.h1.reparentTo(self.track)
	self.h1.setScale(1,1,1)
	self.h1.setPos(-257,-130,1)
	#self.h1.setP(90)
	...
	self.env21 = loader.loadModel("Models/cat-x2-not-yet-classified/bvw-f2004--roadblock-egg/bvw-f2004--roadblock/roadblock")
	self.env21.reparentTo(self.track)
	...
	# let's mark it as a from collider and with its distinct mask as well
		...
		self.bikeCollider.node().addSolid(CollisionSphere(0, 0, 0, 1.7))
	...
	#self.heartModel.reparentTo(render)
		self.bikeModel.setPos(x, y, z)
		self.lightobj.loadchar(self.bikeModel,'jeep')
		if(i%2==0):
			y= k+6
		else:	
			y= k-6
	...
	#self.heartModel.reparentTo(render)
		...
		if(i%2==0):
			...
			self.bikeModel.setPos(x, y, z)
			self.bikeModel.setH(0)
			self.bikeModel.setScale(0.7)
		else:
			self.bikeModel = loader.loadModel("Models/cat-vehicles-road/bvw-f2004--girlscar-egg/bvw-f2004--girlscar/girlcar")
			self.bikeModel.setPos(x, y, z+6)
			self.bikeModel.setH(0)
			self.bikeModel.setScale(1.38)
			self.lightobj.loadchar(self.bikeModel,'blue')
		#self.bikeModel = loader.loadModel("Models/cat-vehicles-land/alice-vehicles--trolley-egg/alice-vehicles--trolley/trolley")
	...
	for i in range(10):
		...
		pandaPosInterval1 = self.bikeModel1[27+i].posInterval(24.5,
                                                        Point3(x, self.bikeModel1[27+i].getY()+900, self.bikeModel1[26+i].getZ()),
 ...
 def setAI(self, entry):
		...
		self.AIchar = AICharacter( "entry", entry, 340, 0.5, 500)
 ...
 def collideEventIn1(self, entry):
  ...
  # we retrieve the two object nodepaths - note that we need to go back the nodes hierarchy because the getXXXNodePath methods returns just the collision geometry, that we know is parented to the very object nodepath we need to manage here
		...
		colliderFROM = entry.getFromNodePath().getParent()
		colliderINTO = entry.getIntoNodePath().getParent()
		colliderFROM.setPos(self.cycle.getPos()+Point3(int(abs(self.cycle.getX()))-400,int(abs(self.cycle.getY()))-400,0))
 ...
 def police(self, task):
		...
		if(((abs(abs(self.cycle.getX())) -(abs(self.pandaActor1.getX()))) < 40) and ((abs((abs(self.cycle.getY()))-(abs(self.pandaActor1.getY()))))<40) ):
			...
			print "Police Around"
			if(((abs(abs(self.cycle.getX())) -(abs(self.pandaActor1.getX()))) < 5) and ((abs((abs(self.cycle.getY()))-(abs(self.pandaActor1.getY()))))<5) ):
				print "Caught"
				click2 = 1
				self.pausekaro('options')
				#self.setnote = self.addInstructions(0.90,"Police Caught\n20$ penalty")
 ...
 def bhagao(self):
	...
	if(click2 == 1):
		...
		self.resume()
		self.speed = 200
		self.pandaActor1.setPos(self.cycle.getPos()+Point3(int(abs(self.cycle.getX()))-300,int(abs(self.cycle.getY()))-300,0))
